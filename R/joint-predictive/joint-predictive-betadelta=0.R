...
simulate_gaussian_data <- function(n_train, n_test, beta_delta, eps) {
  # define the DGP 
...
experiment <- function(exec, n_test, n_train, beta_delta, eps) {
  # simulate the data
  data <- simulate_gaussian_data(n_train, n_test, beta_delta, eps)
...
results_df <- data.frame(
  matrix(
    ...
    0, 
    4, 
    dimnames=list(c(), c("beta_delta", "loo_elpd_diff", "elpd_diff", "eps"))
  )
)
...
for (eps in eps_vals) {
  for (iter in 1:num_iters) {
    experiment_df <- experiment(exec, n_test, n_train, beta_delta, eps)
    results_df <- rbind(results_df, experiment_df)
  }
}

# save the data
#saveRDS(results_df, file = "data/nested_bias_eps.rds")
results_df <- readRDS(file = "data/nested_bias_eps.rds")

# make plot
#cols <- c("1" = "black", "5" = "blue", "10" = "red")
( p <- results_df %>% mutate_at("eps", funs(as.factor)) %>%
    ggplot(aes(x = loo_elpd_diff, y = elpd_diff)) + 
...
save_tikz_plot(p, width = 5, filename = "./tex/snr-bias.tex")
